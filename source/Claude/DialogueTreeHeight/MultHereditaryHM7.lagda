Design H, merged/doubled-budget fork — the `K` combinator, `GoodH-K`, fully
polymorphic. `MultHereditaryH7` retargeted at `MultHereditaryHM`/`HM3`/`HM4`.

`K` consumes a function-typed argument, so the doubled budget propagates: at
every accumulator spot the argument's budget is threaded `2×`
(`kbudget +ℕ kbudget`), and two proofs that were equalities under `1×` become
padded inequalities under `2×` — the `pj` clause (`kJ ka ≤ …` gains one
`bumpk-pad` of `ja′`) and `tail`'s `hj` (`≤-refl` becomes `bumpk-pad`). `K`
stays budget-neutral for `< ε₀`; the extra finite budget is free. Otherwise
this ports verbatim from `H7`.
...
module Claude.DialogueTreeHeight.MultHereditaryHM7
        (fe : Fun-Ext)
       where
...
open import Claude.DialogueTreeHeight.MultHereditaryHM fe
 using (𝕂 ; ZPack ; ZApply ; ZCont ; PackDom ;
        kadd ; kmult ; kbudget)
...
open import Claude.DialogueTreeHeight.MultHereditaryHM3 fe
...
cpack τ σ ka =
 ...
 where
  ...
  body (τ₁ ⇒ τ₂) = λ kb → ccore σ ka (λ w → Z ⊕ kadd τ₁ τ₂ kb w)
                                     (Z ⊕ kmult τ₁ τ₂ kb)
                                     (0 +ℕ (kbudget τ₁ τ₂ kb +ℕ kbudget τ₁ τ₂ kb))
...
ktail-ι (τ₁ ⇒ τ₂) ka A MH JH J hD =
 λ kb → ocore-ι ka (λ w → A w ⊕ kadd τ₁ τ₂ kb w)
         (MH ⊕ kmult τ₁ τ₂ kb)
         (JH +ℕ (kbudget τ₁ τ₂ kb +ℕ kbudget τ₁ τ₂ kb)) J
         (λ w → ≤-trans (hD w)
...
ktail-fn (τ₁ ⇒ τ₂) σ₁ σ₂ ka A MH JH hD hm hj =
 λ kb → ocore-fn σ₁ σ₂ ka (λ w → A w ⊕ kadd τ₁ τ₂ kb w)
         (MH ⊕ kmult τ₁ τ₂ kb)
         (JH +ℕ (kbudget τ₁ τ₂ kb +ℕ kbudget τ₁ τ₂ kb))
         (λ w → ≤-trans (hD w)
                        (⊕-increasing-right (A w) (kadd τ₁ τ₂ kb w)))
         (≤-trans hm (⊕-mono-right ω
                       (⊕-increasing-right MH (kmult τ₁ τ₂ kb))))
         (λ P → ≤-trans (hj P)
                 (bumpk-pad JH (kbudget τ₁ τ₂ kb +ℕ kbudget τ₁ τ₂ kb) P))
 ...
 where
  ...
  body (σ₁ ⇒ σ₂) = λ ka →
   ...
   where
    ...
    -- the doubled argument budget (2×), matching HM.ZApply's arrow clause
    kb2 : 𝕂 (σ₁ ⇒ σ₂) → ℕ
    kb2 ka = kbudget σ₁ σ₂ ka +ℕ kbudget σ₁ σ₂ ka

    pd : (ka : 𝕂 (σ₁ ⇒ σ₂)) (w : 𝓑)
       → (kD (σ₁ ⇒ σ₂) ka w ⊕ Z)
         ≤ ((Z ⊕ (A₁ ka w ⊕ Z))
             ⊗ bumpk (rbb (σ₁ ⇒ σ₂) 0 0 +ℕ (0 +ℕ kb2 ka)) (pool₁ ka))
    pd ka w = ≤-trans (hDin ka w)
               (≤-trans (⊕-increasing-left Z (A₁ ka w))
                        (x-≤-x⊗ (Z ⊕ (A₁ ka w ⊕ Z))
                                (bumpk (rbb (σ₁ ⇒ σ₂) 0 0 +ℕ (0 +ℕ kb2 ka))
                                       (pool₁ ka))
                                (pos₁ ka (rbb (σ₁ ⇒ σ₂) 0 0
                                          +ℕ (0 +ℕ kb2 ka)))))
    ...
    pm ka =
     ...
     where
      ...
      bumpk-pad-left-succ : (ka′ : 𝕂 (σ₁ ⇒ σ₂))
        → bumpk 1 (pool₁ ka′)
          ≤ bumpk ((succ (0 +ℕ 0) +ℕ pr₁ spl) +ℕ kb2 ka′) (pool₁ ka′)
      bumpk-pad-left-succ ka′ =
       transport (λ n → bumpk 1 (pool₁ ka′) ≤ bumpk n (pool₁ ka′))
                 (+ℕ-assoc 1 (pr₁ spl) (kb2 ka′) ⁻¹ ∙ refl)
                 (bumpk-pad 1 (pr₁ spl +ℕ kb2 ka′) (pool₁ ka′))

    pj : (ka : 𝕂 (σ₁ ⇒ σ₂)) (P : 𝓑)
       → bumpk (kJ (σ₁ ⇒ σ₂) ka) P
         ≤ bumpk (rbb (σ₁ ⇒ σ₂) 0 0 +ℕ (0 +ℕ kb2 ka)) P
    pj ka P =
     ≤-trans
      (transport (λ n → bumpk (rbb (σ₁ ⇒ σ₂) ja′ 0) P ≤ bumpk n P)
                 E (≤-refl (bumpk (rbb (σ₁ ⇒ σ₂) ja′ 0) P)))
      (transport (λ n → bumpk (rbb (σ₁ ⇒ σ₂) 0 0 +ℕ ja′) P ≤ bumpk n P)
                 (+ℕ-assoc (rbb (σ₁ ⇒ σ₂) 0 0) ja′ ja′)
                 (bumpk-pad (rbb (σ₁ ⇒ σ₂) 0 0 +ℕ ja′) ja′ P))
     where
      ja′ : ℕ
      ja′ = kbudget σ₁ σ₂ ka
    ...
    tail ka = ktail-fn τ σ₁ σ₂ ka (A₁ ka) (MH₁ ka)
               (0 +ℕ kb2 ka)
               (hDin ka) (hmin ka)
               (λ P → bumpk-pad (kbudget σ₁ σ₂ ka) (kbudget σ₁ σ₂ ka) P)
