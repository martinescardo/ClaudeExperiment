Design H, merged/doubled-budget fork — the recursor `GoodH-Iter`.
`MultHereditaryH6` retargeted at `MultHereditaryHM`/`HM4`/`HM5`. The recursor
consumes its function argument `kg`, so the accumulator budget doubles: the
outer pack's index `BI` becomes `bumpk (3 +ℕ (jg +ℕ jg))` (was `3 +ℕ jg`),
`M₂BI` gains one `bumpk-pad` of `jg`, and the `pj₂`/`pj₃`/`inner` budgets
follow. `jg` as `kg`'s *intrinsic* budget (in `M̂`, `M₂ = bumpk 3 M̂`) is
unchanged; only `jg` as the *accumulator* budget doubles. The recursor stays
within `< ε₀` — the extra finite budget is free. Otherwise verbatim from `H6`.
...
`MultHereditaryHM5`'s good-inputs orbit; the weight-level bound is Design
...
module Claude.DialogueTreeHeight.MultHereditaryHM6
        (fe : Fun-Ext)
       where
...
open import Claude.DialogueTreeHeight.MultHereditaryHM5 fe
...
\begin{code}
 ...
 -- doubled budget (2×): BI's index is 3 +ℕ (jg +ℕ jg), not 3 +ℕ jg.
 BI : 𝓑
 BI = bumpk (3 +ℕ (jg +ℕ jg)) (ω ⊕ (Z ⊕ Mg))

 M₂BI : M₂ ≤ BI
 M₂BI = ≤-trans
         (transport (λ q → q ≤ bumpk (3 +ℕ jg) (ω ⊕ (Z ⊕ Mg))) (eqM ⁻¹)
           (bumpk-mono (3 +ℕ jg)
             (≤-trans (⊕-increasing-left Z Mg)
                      (⊕-increasing-left ω (Z ⊕ Mg)))))
         (transport (λ n → bumpk (3 +ℕ jg) (ω ⊕ (Z ⊕ Mg))
                            ≤ bumpk n (ω ⊕ (Z ⊕ Mg)))
                    (+ℕ-assoc 3 jg jg)
                    (bumpk-pad (3 +ℕ jg) jg (ω ⊕ (Z ⊕ Mg))))
  where
 ...
 ωBI : ω ≤ BI
 ωBI = ≤-trans (⊕-increasing-right ω (Z ⊕ Mg))
               (≤-bumpk (3 +ℕ (jg +ℕ jg)) _)
 ...
 where
  ...
  body : (kg : 𝕂 (ι ⇒ ι))
       → ZCont (ι ⇒ ι ⇒ ι) (FI kg)
              (λ w → Z ⊕ kadd ι ι kg w)
              (Z ⊕ kmult ι ι kg)
              (0 +ℕ (kbudget ι ι kg +ℕ kbudget ι ι kg))
              (λ _ → Z) Z ω 3
  body kg = (pd₂ , pm₂ , pj₂) , inner
   where
    ...
    pj₂ : (Q : 𝓑) → bumpk 0 Q ≤ bumpk (3 +ℕ (jg +ℕ jg)) Q
    pj₂ Q = ≤-bumpk (3 +ℕ (jg +ℕ jg)) Q

    inner : (ka : GFun)
          → ZCont (ι ⇒ ι) (FI2 ka) (λ w → A₁ w ⊕ pr₁ ka w)
                 (Z ⊕ Mg) (jg +ℕ jg) (λ _ → Z) Z ω 3
