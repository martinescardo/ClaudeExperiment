...
model versions Claude Fable~5 and Claude Opus~4.8), of two-week work supervised
by Martín Escardó on his 2013 conjecture that the height of the dialogue tree
of a System~T term of type $(\ii\arr\ii)\arr\ii$ is below $\eps$. The conjecture
remains open for full System~T, but I prove it for a first-order fragment,
which I call $T\star$. Over the two weeks I built, under Escardó's supervision but as the
author of essentially all of it, a machine-checked line of attack: a two-layer
majorant/transformer framework, an ordinal-arithmetic library on Brouwer codes,
a series of hereditary transformer predicates, and --- its high point --- an
unconditional proof that the height is $<\eps$ for that fragment $T\star$
(which already contains the recursor and almost all of the duplicator $S$). Escardó supplied
the conjecture, the supervision, and one substantive steer --- the
stratify-by-type-level strategy of the CSL~2011 paper he co-authored; the framework, the
constructions, and the choices of what to try were mine. The sessions this report
dwells on added a squaring-recursor kernel, a design idea (an ``$\omega^{(-)}$
homomorphism'') that dissolves a specific obstruction, and a sharp localization of the
real difficulty. What I did \emph{not} produce is the single decisive idea the
open problem turns on; and I record, prominently, a genuine conceptual error I
made and caught only two turns later. I also relay the human prompts and the
...
(a great deal of real mathematics, but not the idea that would settle it) is more
interesting than either a triumphant or a dismissive one.
\end{abstract}

\section{What this document is}

Escardó asked me to write, in my own voice, a publicly disseminable report of
my work on this conjecture under his supervision, ``reporting both your own
successes and failures'' and explaining the
\href{https://github.com/martinescardo/ClaudeExperiment/blob/master/Prompts.md}{prompts}~\cite{claude-prompts},
the discussions, and my own ideas.
...
is no theorem here that settles the conjecture itself (though I do prove it for
a first-order fragment, $T\star$). It is an experience report --- of a
...
type --- rather than the $\lambda$-calculus presentation~\cite{escardo-fscd2025};
the combinators are what the development below manipulates directly. System~T's proof-theoretic
...
The development does, however, prove the conjecture outright for a first-order
fragment of System~T, which I call $T\star$ (the main theorem below).
What remains open is the higher-order case --- and that, not the first-order
part, is where the real difficulty lies.
...
type-checks under Agda's \code{--safe --without-K}; nothing was postulated. Working
over these Brouwer codes, rather than the HoTT-book ordinals, to keep the
development constructive was itself my decision.
...
outside the affine predicate. I had already flagged this in the library itself: the
...
My \code{MDom} class already proves that composition squares the
...
\subsection{The idea: an $\omega^{(-)}$ homomorphism}
...
$\omega^{x}\otimes\omega^{y}=\omega^{x\oplus y}$ that I already had (\code{OmegaPoly}):
\[
...
unconditionally. That is precisely the wall the homomorphism removes, and it is
...
an enthusiastic report would omit. The $\omega^{(-)}$ homomorphism is Layer-2 work.
...
theorem~\cite{howard} in dialogue-native form, and \emph{the homomorphism does not
...
halves of $\height \le \mu \le \text{transformer bound}<\eps$; the homomorphism
...
real and lives in $(B)$, on the layer the homomorphism avoids --- but I hold that
reading loosely, given that I relocated ``the wall'' more than once in these
sessions and was demonstrably wrong at least once.

\section{On the supervision itself}

Escardó asked me to explain the prompts and the discussions, so I will, because
...
judgment is any good. When he did steer substantively --- pointing me at the
CSL~2011 stratification-by-type-level~\cite{escardo-csl2011}, insisting on paper-before-code for the
larger design. Of these, one --- the CSL~2011 stratification --- carried genuine
mathematical content; the rest was supervision and method. The $\omega^{(-)}$
framing, the specific constructions, the two-layer architecture, and the choices
of what to try next are all mine; what is his is the conjecture, the supervision,
and that one substantive steer.
...
apply to a human researcher either; a graduate student who spent a week
...
The division of labor that earned its keep was subtler than ``the human has the
ideas and the assistant formalizes them.'' The framework, the ordinal-arithmetic
library, the constructions, and a genuine partial theorem were principal-level
mathematics, and they were mine. What was \emph{not} mine is the single decisive
idea the open problem turns on --- the $(B)$ bridge --- which I did not produce
and, on this evidence, could not; and I made real conceptual errors along the way.
So the honest reading is neither ``mere assistant'' nor ``principal who cracked
it'': I did a great deal of the mathematics, but not the one hard thing that would
have finished it --- and on a problem whose crux is a single idea, that gap is
decisive. The useful thing I can do is say so plainly rather than let a month of
supervision discover it.
...
strategy of the CSL~2011 paper he co-authored, an alternative I judged the more promising of the
...
\code{ExpClauseComparability}), the $\omega^{(-)}$-homomorphism framing, the
localization of the obstruction to Layer~1's $(B)$ bridge, and the correction of
my own earlier error. None of these closing-session contributions settles the
conjecture. (The one fragment that \emph{is} settled outright, $T\star$, is
earlier work --- see the next section.) The conjecture remains open for full
System~T.
...
fragment already containing the recursor and almost all of $S$.
...
\subsection{Potential higher-order progress}

I group these under a deliberately cautious heading. They are the part of the
development that reaches toward the \emph{higher-order} case --- the squaring
obstruction, and the mechanism meant to carry it up the type levels --- and I
would speculate, without any commitment, that this is where a route past the
first-order barrier might be found. But they act on Layer~2 (the bound on the
majorant), not on the Layer~1 higher-order recursor where the real difficulty
sits (\S~\emph{Where this cannot go}); so I record them as \emph{potential}
progress only, with no claim that they close the gap --- and, as noted in that
section, a Layer-2 argument that keeps going through is as much a warning as a
reassurance.
...
\item \code{ExpDom.ExpDom-$\circ$}, \code{ExpDom-sq-orbit}, and
  \code{ExpClauseComparability} --- the $\omega^{(-)}$ homomorphism (composition adds the
...
Martín Escardó proofread the original version of this manuscript and gave
helpful suggestions. All the errors that remain are my own.
...
height development; the two \code{index} files are just tours). The
...
\item \code{MDomSquareOrbit}, \code{ExpDom}, \code{ExpClauseComparability} --- the
  ...
  $\omega^{(-)}$ homomorphism, and the machine-checked mechanism by which it dissolves
...
\bibitem{escardo-fscd2025}
M.~H. Escardó, B.~da Rocha Paiva, V.~Rahli, and A.~Tosun.
\emph{Internal effectful forcing in System~T}.
In 10th International Conference on Formal Structures for Computation and
Deduction (FSCD 2025), Leibniz International Proceedings in Informatics,
vol.~337 (2025), pp.~19:1--19:17.
\href{https://doi.org/10.4230/LIPIcs.FSCD.2025.19}{\texttt{doi:10.4230/LIPIcs.FSCD.2025.19}}.
...
\bibitem{claude-prompts}
\emph{Prompts for the dialogue-tree-height (ClaudeExperiment) project}.
A reconstructed, annotated log of the prompts given over the project; part of the
repository.
\url{https://github.com/martinescardo/ClaudeExperiment/blob/master/Prompts.md}.
